// Enhanced Hippodrome Solution Explorer - Multi-Target JavaScript
class EnhancedHippodromeExplorer {
    constructor() {
    ...
    initializeElements() {
        ...
        this.closeComparison = document.getElementById('close-comparison');
    ...
    bindEventListeners() {
        ...
        this.compareBtn.addEventListener('click', () => this.showTargetComparison());
        ...
        // Modal controls
        this.closeComparison.addEventListener('click', () => this.hideComparison());
        ...
        try {
            ...
            if (data.targets) {
                ...
                if (this.availableTargets.length > 0) {
                    const defaultTarget = this.availableTargets.find(t => t.name === 'top-row') || this.availableTargets[0];
    ...
    onTargetChange() {
        ...
        // If we have a current solution, try to load the same configuration for the new target
        if (this.currentSolution) {
            this.searchById(this.currentSolution.id);
        } else {
            this.loadRandomSolution();
        }
    }
    ...
    async showTargetComparison() {
        const configId = this.currentSolution?.id;
        
        if (!configId) {
            this.showError('Please load a solution first to compare targets');
            return;
        }
        
        try {
            const response = await fetch(`/api/compare_targets/${configId}`);
            const data = await response.json();
            ...
            this.showError('Failed to load target comparison');
            console.error('Error:', error);
        }
    }

    displayComparison(data) {
        let html = `<h4>Configuration ${data.config_id} - Target Comparison</h4>`;
        html += '<div class="comparison-grid">';
        
        data.comparisons.forEach(comp => {
            const difficulty = comp.moves === -1 ? 'Unsolvable' : `${comp.moves} moves`;
            const time = comp.moves === -1 ? '-' : `${comp.time_ms.toFixed(2)}ms`;
            
            html += `
                <div class="comparison-item ${comp.target_name === this.currentTarget ? 'current-target' : ''}">
                    <h5>${comp.target_name}</h5>
                    <div class="comparison-positions">Positions: ${comp.target_positions.join(', ')}</div>
    ...
    // Playback control methods (similar to original but updated for target support)
    togglePlayback() {
        if (this.isPlaying) {
            this.stopPlayback();
        } else {
    ...
    // Editor methods (similar to original)
    toggleEditMode() {
...
document.addEventListener('DOMContentLoaded', () => {
    window.explorer = new EnhancedHippodromeExplorer();
}); 